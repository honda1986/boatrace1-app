...
if run:
    ...
    # ── 回収率計算 (過去日付のみ) ──
    if is_past:
        # 三連単 1-全-2 = 1-3-2, 1-4-2, 1-5-2, 1-6-2 の4点 × 100円 = 400円/レース
        bet_per_race = 400
        ...
        for race in hit_list:
            ...
            # 1着が1号艇 かつ 3着が2号艇 なら的中
            if len(fo) >= 3 and fo[0] == 1 and fo[2] == 2:
                total_payout += payout
                race["is_hit"] = True
                hit_count += 1
            else:
        ...
        st.markdown(f"""
        <div class="roi-box">
            <div style="color:#aaa;font-size:0.9em;">三連単 1-全-2 (4点×100円) 回収率</div>
